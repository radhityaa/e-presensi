...
let passWordConf = document.getElementById('password-confirm-visibility');

if (passWordConf) {
    passWordConf.addEventListener('click', passwordConfFunction);
}

function passwordConfFunction() {
    let passInputConfrm = document.getElementById('password_confirmation');
    passWordConf.classList.toggle('active');

    if (passInputConfrm.type === 'password') {
        passInputConfrm.type = 'text';
    } else {
        passInputConfrm.type = 'password';
    }
}
